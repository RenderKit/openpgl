...
void SampleView::loadSampleData()
{
    ...
    if(m_sampleStorage) {
      ...
      std::cout << "AABB: min = " << m_sampleBounds.min[0] << " " << m_sampleBounds.min[1] << " "<< m_sampleBounds.min[2] << "\t max = " << m_sampleBounds.max[0] << " "<< m_sampleBounds.max[1] << " "<< m_sampleBounds.max[2] << std::endl;
